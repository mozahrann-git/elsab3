    ...
    try {
      ...
      r.onstop = async () => {
        ...
        try {
          ...
          setState('done');
        } catch (e: any) { setErr(`الرفع فشل: ${e?.message || 'جرب تاني'}`); setState('err'); }
      };
      r.start(); rec.current = r; setState('rec');
    } catch { setErr('المتصفح مسمحش بالمايك — اسمح للميكروفون من إعدادات الموقع'); setState('err'); }
  };
  return (
    ...
    <button type="button" onClick={() => (state === 'rec' ? rec.current?.stop() : start())} disabled={state === 'up'}
      ...
      {state === 'rec' ? <><Square size={18} />وقّف التسجيل</> : state === 'up' ? 'جاري الرفع...' : state === 'done' ? '✓ الفويس اتسجل (دوس تسجّل تاني)' : state === 'err' ? 'مش قادر يسجل — دوس تجرب تاني'  : <><Mic size={18} />سجّل فويس</>}
    </button>
