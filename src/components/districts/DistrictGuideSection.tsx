...
export const DistrictGuideSection: React.FC<{
  ...
  return (
    <section id="districts-guide-section" className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 my-12 overflow-x-hidden" dir="rtl">
      <div className="bg-[#141414] text-white rounded-[24px] sm:rounded-[28px] overflow-hidden w-full">
        <div className="p-4 sm:p-10 lg:p-12 grid gap-6 sm:gap-8 lg:grid-cols-12 min-w-0">
          {/* العنوان والملخص */}
          <div className="lg:col-span-4 space-y-4 sm:space-y-5 min-w-0 lg:sticky lg:top-24 self-start">
            <span className="text-xs font-bold tracking-wide text-[#D9B864]">دليل أحياء الهضبة الوسطى</span>
            <h2 className="text-2xl sm:text-4xl font-bold leading-tight font-readex">كل حي بسعره،<br />قبل ما تنزل تعاين.</h2>
          ...
          <div className="lg:col-span-8 space-y-2.5 min-w-0">
            <div className="hidden sm:grid grid-cols-12 text-[11px] text-[#7C7870] px-4 pb-1 min-w-0">
            ...
            {rows.map((s, i) => {
              ...
              return (
                <button key={s.name} onClick={() => onOpen(s.name)}
                  className="group w-full text-right rounded-2xl bg-white/[0.04] hover:bg-white/[0.09] border border-white/10 hover:border-[#D9B864]/50 transition p-3.5 sm:p-4 flex flex-col sm:grid sm:grid-cols-12 gap-3 sm:items-center min-w-0">
                  <div className="sm:col-span-4 flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[11px] text-[#A3A09A] truncate">{c?.tagline || `${s.count} شقة متاحة · تبدأ من ${m(s.minPrice)}`}</p>
                    </div>
                    <ArrowUpLeft size={16} className="text-[#D9B864] shrink-0 sm:hidden" />
                  </div>
                  <div className="sm:col-span-5 space-y-1.5 min-w-0">
                    <div className="flex items-baseline justify-between gap-2 min-w-0">
                      <span className="font-bold font-readex whitespace-nowrap">{f(s.avgPpm)} <span className="text-[10px] font-normal text-[#A3A09A]">ج.م/م²</span></span>
                      <span className={`text-[11px] font-bold whitespace-nowrap ${vs <= 0 ? 'text-[#7ED3A0]' : 'text-[#F0A48F]'}`}>{vs <= 0 ? '▼' : '▲'} {Math.abs(vs).toFixed(0)}% عن المتوسط</span>
                    </div>
                    <div className="h-1.5 w-full rounded-full bg-white/10 overflow-hidden">
                      <div className="h-full rounded-full bg-linear-to-l from-[#D9B864] to-[#A07A26] transition-all duration-700" style={{ width: `${Math.max(8, (s.avgPpm / top) * 100)}%` }} />
                    </div>
                  </div>
                  <div className="hidden sm:col-span-3 sm:flex items-center justify-end gap-2">
                    <div className="text-left">
                      <p className="text-sm font-bold whitespace-nowrap">{m(s.minPrice)}</p>
                      <p className="text-[10px] text-[#A3A09A]">{s.count} شقة</p>
                    </div>
                    <ArrowUpLeft size={16} className="text-[#D9B864] opacity-0 group-hover:opacity-100 transition" />
                  </div>
                </button>
              );
            })}
